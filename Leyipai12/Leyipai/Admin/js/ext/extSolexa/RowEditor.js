        ...
        if(this.el && (this.isVisible() || force === true)){
            ...
            for(var i = 0, len = cm.getColumnCount(); i < len; i++){
                if(!cm.isHidden(i)){
                    var adjust = 0;
                    if(i == 0){
                        adjust += (Ext.isIE ? 5:0); // outer padding
                    }
                    if(i == (len - 1)){
                        adjust += (Ext.isIE ? 0:3); // outer padding
                    } else{
                        adjust += (Ext.isIE ? -1:1);
                    }
                    fields[i].show();
                    fields[i].setWidth(Ext.isIE ?cm.getColumnWidth(i) - adjust:cm.getColumnWidth(i) - adjust-1);
                } else{
                    fields[i].hide();
                }
            }
    ...
    showTooltip: function(msg){
        ...
        if(!t){
            t = this.tooltip = new Ext.ToolTip({
                ...
                cls: 'errorTip',
                width: 300,
                title: "<font style='font-size:12px'>错误</font>",
                id:'showTooltip',
                autoHide: false,
                anchor: 'left',
                anchorToTarget: true,
        ...
        this.items.each(function(f){
	            if(!f.isValid(true)){
	                t.initTarget(f.getEl());
	                flag = true ;
	            }
	    }, this);
        this.items.each(function(f){
            if(String(this.values[f.id]) != String(f.getValue())&&!flag){
                t.initTarget(f.getEl());
            }
        }, this);
