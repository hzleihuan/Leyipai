...
{
	if(!$(module_id)) return;
	$(module_id+"_title").style.width=$(module_id+"_head").scrollWidth - $(module_id+"_text").scrollWidth - 120 - 40 + "px";
	$(module_id+"_title").style.width=$(module_id+"_head").scrollWidth - $(module_id+"_text").scrollWidth - 120 - 40 + "px";
}
...
{
   $("module_"+module_id+"_ul").innerHTML="<div style='text-align:center;'><img src='/images/loading.gif' align='absMiddle'> 加载中，请稍候……</div>";
   var xmlHttpObj=getXMLHttpObj();
   ...
   xmlHttpObj.onreadystatechange=function()
	{
	 	if(xmlHttpObj.readyState==4)
	 	{
	 		if(xmlHttpObj.status==200)
	 		   ...
	 		   $("module_"+module_id+"_ul").innerHTML="更新内容错误，代码："+xmlHttpObj.status;
	 	}
	}
   xmlHttpObj.send(null)
}
...
function NextPage(module_id, page)
{
   ...
   if(o.scrollTop > 0)
      $('module_'+module_id+'_link_pre').className="";
   else
      $('module_'+module_id+'_link_pre').className="PageLinkDisable";
   if(o.scrollHeight-o.scrollTop>lines_per_page(module_id)*20)
      $('module_'+module_id+'_link_next').className="";
   else
